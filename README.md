...
### 4. First-time setup — create an admin user

Use the Swagger UI at `http://localhost:8000/docs` or `curl` to register the first user:
...
Then log in via the UI or obtain a token directly:

```bash
curl -X POST http://localhost:8000/auth/login \
  -F "username=admin@hospital.com" \
...
- `require_role()` dependency factory enforces RBAC at the endpoint level.
...
| `doctor` | Full CRUD | Read + Assign | Read + Ingest | Read + Upload | Read + Create | Read + Create | Read own |
| `nurse` | Read only | Read only | Read only | Read + Upload | Full CRUD | Read | Read own |
...
- Ensure the user was registered with `is_active = true` (default).
