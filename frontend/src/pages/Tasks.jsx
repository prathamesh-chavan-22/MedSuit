...
const EMPTY = { patient_id: "", assigned_to: "", shift_id: "", title: "", description: "", priority: 1 };
...
export default function Tasks() {
  ...
  const { data: shifts = [] } = useQuery({ queryKey: ["shifts"], queryFn: () => api.get("/tasks/shifts").then(r => r.data) });

  const createMut = useMutation({
    mutationFn: (d) => api.post("/tasks/", d),
    onSuccess: () => { qc.invalidateQueries(["tasks"]); setShowForm(false); setForm(EMPTY); },
  });
    ...
    <div style={styles.page}>
      ...
      {showForm && (
        <div style={styles.formCard}>
          ...
          <form onSubmit={e => { e.preventDefault(); createMut.mutate({ ...form, patient_id: Number(form.patient_id), assigned_to: form.assigned_to ? Number(form.assigned_to) : null, shift_id: form.shift_id ? Number(form.shift_id) : null, priority: Number(form.priority) }); }} style={styles.grid2}>
            <div style={styles.field}>
              <label style={styles.label}>Patient *</label>
              <select required value={form.patient_id} onChange={e => setForm(p => ({ ...p, patient_id: e.target.value }))} style={styles.input}>
            ...
            <div style={styles.field}>
              ...
              <select value={form.shift_id} onChange={e => setForm(p => ({ ...p, shift_id: e.target.value }))} style={styles.input}>
            ...
            <div style={{ ...styles.field, gridColumn: "1/-1" }}>
              ...
              <input required value={form.title} onChange={e => setForm(p => ({ ...p, title: e.target.value }))} style={styles.input} placeholder="e.g. Check vital signs" />
            </div>
            <div style={{ ...styles.field, gridColumn: "1/-1" }}>
              <label style={styles.label}>Description</label>
              <input value={form.description} onChange={e => setForm(p => ({ ...p, description: e.target.value }))} style={styles.input} placeholder="Optional details…" />
            </div>
            <div style={styles.field}>
            ...
            <div style={{ gridColumn: "1/-1", display: "flex", gap: 10 }}>
              <button type="submit" style={styles.btn} disabled={createMut.isPending}>{createMut.isPending ? "Saving…" : "Save"}</button>
      ...
      <div style={styles.tabs}>
        {["all", "pending", "in_progress", "done"].map(s => (
          <button key={s} style={{ ...styles.tab, ...(filter === s ? styles.tabActive : {}) }} onClick={() => setFilter(s)}>
            ...
            <span style={styles.tabCount}>{s === "all" ? tasks.length : tasks.filter(t => t.status === s).length}</span>
          </button>
        ))}
      </div>
      ...
      <div style={styles.list}>
        ...
        {filtered.map(task => {
          ...
          return (
            <div key={task.id} style={{ ...styles.taskCard, opacity: task.status === "done" ? 0.6 : 1 }}>
              <button style={styles.iconBtn} onClick={() => updateMut.mutate({ id: task.id, status: nextStatus[task.status] })} title="Cycle status">
              ...
              <div style={{ flex: 1 }}>
                ...
                <div style={styles.taskMeta}>
                  ...
                  {" · "}
                  <span style={{ color: priorityColor[task.priority] }}>{priorityLabel[task.priority]} priority</span>
                </div>
              </div>
              <button style={styles.btnDanger} onClick={() => deleteMut.mutate(task.id)}>✕</button>
            </div>
          );
        })}
      </div>
...
const styles = {
  ...
  btnDanger: { background: "none", color: "#9ca3af", border: "none", cursor: "pointer", fontSize: 16, padding: 4 },
  ...
  tab: { background: "#f3f4f6", border: "none", borderRadius: 8, padding: "7px 14px", fontSize: 13, fontWeight: 500, cursor: "pointer", display: "flex", alignItems: "center", gap: 6 },
  tabActive: { background: "#3b82f6", color: "#fff" },
  tabCount: { background: "rgba(0,0,0,0.12)", borderRadius: 99, padding: "1px 6px", fontSize: 11 },
  list: { display: "flex", flexDirection: "column", gap: 10 },
  empty: { color: "#9ca3af" },
  taskCard: { background: "#fff", borderRadius: 10, padding: "14px 16px", display: "flex", alignItems: "flex-start", gap: 12, boxShadow: "0 1px 4px rgba(0,0,0,0.06)" },
